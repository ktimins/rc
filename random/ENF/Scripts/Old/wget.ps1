...
if ($zip -or $zd) {
   ...
   $filesToZip = Get-ChildItem -Recurse -Path $filePath | Where-Object {$_.Extension -eq ".zip" }

   $zipFile = "$enf.zip"

   rm -ErrorAction SilentlyContinue "$filePath\$enf.zip"

   Get-ChildItem -Path $filePath | Where-Object {$_.Name -ne "$ENF.zip" } | Add-Zip "$filePath\$enf.zip"
